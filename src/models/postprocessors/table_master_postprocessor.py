...
class TableMasterPostprocessor(BaseTextRecogPostprocessor):
    ...
    def _get_pred_bbox_mask(self, strings: List[str]) -> np.ndarray:
        ...
        """
        pred_bbox_masks = []
        sos_token = self.dictionary.idx2str([self.dictionary.start_idx])
        eos_token = self.dictionary.idx2str([self.dictionary.end_idx])  
        pad_token = self.dictionary.idx2str([self.dictionary.padding_idx])
