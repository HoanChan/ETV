...
class TableLoss(nn.Module):
    ...
    Args:
        loss_text (dict): Config for text loss module.
        loss_bbox (dict): Config for bbox loss module.
    """
    def __init__(self, loss_text, loss_bbox):
        super().__init__()
    ...
    def forward(self, 
        ...
        Returns:
            dict: Dictionary with computed losses 'loss_text' and 'loss_bbox'.
        """
        gt_texts = [s.gt_text for s in data_samples]
        gt_bboxes = [s.gt_bbox for s in data_samples]

        loss1 = self.loss_text(outputs[0], gt_texts)
        loss2 = self.loss_bbox(outputs[1], gt_bboxes)
        return dict(loss_text=loss1, loss_bbox=loss2)
